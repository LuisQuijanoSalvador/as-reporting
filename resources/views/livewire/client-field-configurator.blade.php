        ...
        @if ($selectedClientId)
            ...
            {{-- @foreach ([1, 2, 3, 4] as $i)
                <div style="border: 1px solid #ccc; padding: 10px; margin-bottom: 10px;">
                    <h4>Configuración de **Cod{{ $i }}**</h4>
                    
                    <div>
                        <input type="checkbox" id="visible{{ $i }}" 
                               wire:model.live="config.cod{{ $i }}_is_visible">
                        <label for="visible{{ $i }}">Mostrar en Reporte</label>
                    </div>

                    <div>
                        <label for="label{{ $i }}">Etiqueta (Título de Columna):</label>
                        <input type="text" id="label{{ $i }}" 
                               wire:model.live="config.cod{{ $i }}_label" 
                               placeholder="Ej: Centro de Costo">
                        @error("config.cod{{ $i }}_label") <span style="color: red;">{{ $message }}</span> @enderror
                    </div>
                </div>
            @endforeach --}}
            <div style="border: 1px solid #ccc; padding: 10px; margin-bottom: 10px;">
                <h4>Configuración de **Cod1**</h4>
                <div>
